...
**Overwriting unrelated scrapings** - The scraper is able to find scrapings by matching folder names with your queries. If your queries are complex, the folder name will be truncated as there are only so many characters that can be saved to a folder's name. This truncation can become an issue if you have two very long queries that look nearly identical but differ in small details found in the truncated section as the scraper may think the two queries are the same and treat them as such (like adding to the wrong partial file. Though rare, it is important to be cognizant of it. There is a feature in the scraper GUI options to use a new file name instead. This is a wonderful way to track which scraping is which. However, the scraper will only continue where it left off if there is a matching named folder, so make sure you are typing in the same name again to start where you left off. This also means that different scrapings can be combined in the same folder if you name them the same name (and this is NOT recommended)

**File name too long** - The naming of the folder structure is suited for Mac's character limit. I have found while trying to access scraped files on a Windows machine that files cannot be opened if the parent directory has too long of a folder name. I am unsure if this would also be a problem for saving the scraped file while using a Windows-installed scraper. One workaround is to use the file renaming option found in the GUI option tab which allows you to choose the name of the folder that the scraped files are saved to. Please note that the issue with overwriting unrelated scrapings (detailed above) can occur if you name two unrelated scrapings with the same file name.
...
**Issues or crashes on Windows or Linux** Although the scraper has been tested on Windows machines, I cannot guarantee everything will work as it was originally built on a Mac. Please make sure you are specifically installing the Windows or Linux version rather than executing the Mac executable. If all else fails, it might be prudent to skip the GUI and enter queries via eHRAF_Scraper.py 
